...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_size', type=int, default=9, help='Number of instances to load')
    parser.add_argument('--load_file', type=str, default='../data/cvrptw/cvrptw20_test_seed1234.pkl', help='File to load instances from')
    parser.add_argument('--capacity_scale', type=int, default=30, help='Capacity of the vehicles')
    ...
    logging.info(f'dataset: {load_file}')
    ...
    for data_index in range(load_size):
        ...
        for sample_num in range(1):
            logging.info(f'第{sample_num}次采样')
            # 初始化数据
            ...
            list.append(objective_value)
        list_2d.append(list)
    # 将list_2d保存到txt文件中
    with open('OR-Tools-CVRPTW-sample.txt', 'w') as f:
        for list in list_2d:
